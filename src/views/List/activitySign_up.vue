<template>
  <div class="activityDetail">
    <div class="weui-panel weui-panel_access">
      <div class="left">已报名人数</div>
      <div class="right">{{count||0}}/{{countAll||0}}人</div>
    </div>
    <div class="wrapper" ref="wrapper">
            ...
            <div class="weui-media-box weui-media-box_small-appmsg">
              <div class="weui-cells">
                <a class="weui-cell weui-cell_access" href="javascript:;">
                  <div class="weui-cell__hd" style="position:relative"><img  :src="item.enroll_pic_url" :onerror="logo">
                    <span class="family-count">{{item.family_num}}</span>
                  </div>
                  <div class="weui-cell__bd weui-cell_primary">
                    <p>{{item.enroll_name}}</p>
                    <p>{{item.phone}}</p>
                  </div>
                  <span class="weui-cell__ft enroll_time">
                        {{item.enroll_time_f}}
                      </span>
                </a>
              </div>
      ...
      <div id="rec-list" v-if="data.length===0">
        <div id="comment-list">
  ...
  export default {
    ...
    methods: {
      getPage () {
        ...
        http
          .post('/index.php?app=activity&m=act&cmd=105', {
            data: obj
          })
          .then((res) => {
            if (Number(res.errcode) === 0) {
              for (let index in res.info.data) {
                // 计算报名人员
                this.countAll = this.countAll + Number(res.info.data[index].family_num)
              }
            }
          })
      },
        ...
        http
          ...
          .then(res => {
            ...
            for (let index in this.data) {
              // 计算报名人员
              count = count + Number(this.data[index].family_num)
              this.data[index].enroll_time_f = moment
                .unix(Number(this.data[index].enroll_time))
